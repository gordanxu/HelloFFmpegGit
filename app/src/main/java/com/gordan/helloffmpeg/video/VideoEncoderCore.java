...
package com.gordan.helloffmpeg.video;
...
public class VideoEncoderCore {
    ...
    public VideoEncoderCore(int width, int height, int bitRate, String path)
        ...
        /****xpzhi  MediaCodec创建了输入的表面surface 并将输入的表面传递出去 用于接收外部的相机预览画面****/
...
//                    encodedData.limit(mBufferInfo.offset + mBufferInfo.size);
                    ...
                    /***xpzhi 通过复用器将视频帧写入文件 encodedData也即是一帧视频数据 (注意这里的数据存放于本地的堆内存中并不在JVM中)***/
                    if (mMuxerStarted) {
    ...
    class AudioEncoder implements Runnable {
        ...
        private boolean audioStep() throws IOException {
            ...
            if (index >= 0) {
                ...
                if (length > 0) {
                    if (baseTimeStamp != -1) {
                        //xpzhi 从上次断开的时间继续录制 如果是结束就传入 流结束的标记位
                        ...
                        //xpzhi 向音频编码器中（其实就是一个队列 ，将数据存入队列中）写入数据，如果结束就传入 流结束的标记位
            ...
            do {
                ...
                if (outIndex >= 0) {
                    ...
                    if (mMuxerStarted && mInfo.presentationTimeUs > 0) {
                        try {
                            /****xpzhi 通过复用器将一帧音频数据写入文件 buffer也即是音频数据（音频数据存放在本地的堆内存中并不在JVM虚拟机中）****/
